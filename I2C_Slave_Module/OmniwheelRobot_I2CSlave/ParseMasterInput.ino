...
/*
void getLatestMasterData()
{
  static unsigned long lastRecieved;
 I2CFlushUnilLatest(); // get rid of all the data except the most recent two bytes.
...
// THIS IS WHERE I2C Data Goes!
void handleReceipts(int numOfBytes) {
    if (Wire.available()) {
        byte masterCommand = Wire.read();
        processTheByte(masterCommand);
        }
    }
